...
SCRIPT_OUTPUT_DIR = "conservative_combo_submit_scripts"
...
def default_batch_size(num_models: int) -> int:
    if num_models <= 2:
        return 12
    if num_models <= 4:
        return 8
...
SAVE_PATH="${{SAVE_PATH:-checkpoints/meta_models/conservative_combo/{index}_{combo_tag}.pth}}"
...
exec torchrun --standalone --nproc_per_node="${{NPROC_PER_NODE}}" "${{PROJECT_ROOT}}/train_conservative_meta_with_bases.py" \\
        ...
        manifest_rows.append(
            {
                ...
                "default_save_path": f"el-mlffs/checkpoints/meta_models/conservative_combo/{index}_{'_'.join(combo)}.pth",
            }
        )
