...
DONE_FILE="${BASE_OUTPUT_DIR}/.dp.done"
CKPT_PATH="${BASE_OUTPUT_DIR}/dp_torch.pth"
LOG_FILE="${BASE_LOG_DIR}/1_dp.log"

if [[ -f "${DONE_FILE}" && -f "${CKPT_PATH}" ]]; then
  echo "Skip dp: completed marker exists at ${DONE_FILE}"
  exit 0
fi
