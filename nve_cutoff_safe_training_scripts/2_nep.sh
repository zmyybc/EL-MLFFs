...
DONE_FILE="${BASE_OUTPUT_DIR}/.nep.done"
CKPT_PATH="${BASE_OUTPUT_DIR}/nep_torch.pth"
LOG_FILE="${BASE_LOG_DIR}/2_nep.log"

if [[ -f "${DONE_FILE}" && -f "${CKPT_PATH}" ]]; then
  echo "Skip nep: completed marker exists at ${DONE_FILE}"
  exit 0
fi
