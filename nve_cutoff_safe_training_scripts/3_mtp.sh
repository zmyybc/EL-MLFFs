...
DONE_FILE="${BASE_OUTPUT_DIR}/.mtp.done"
CKPT_PATH="${BASE_OUTPUT_DIR}/mtp_torch.pth"
LOG_FILE="${BASE_LOG_DIR}/3_mtp.log"

if [[ -f "${DONE_FILE}" && -f "${CKPT_PATH}" ]]; then
  echo "Skip mtp: completed marker exists at ${DONE_FILE}"
  exit 0
fi
