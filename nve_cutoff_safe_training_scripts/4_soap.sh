...
DONE_FILE="${BASE_OUTPUT_DIR}/.soap.done"
CKPT_PATH="${BASE_OUTPUT_DIR}/soap_torch.pth"
LOG_FILE="${BASE_LOG_DIR}/4_soap.log"

if [[ -f "${DONE_FILE}" && -f "${CKPT_PATH}" ]]; then
  echo "Skip soap: completed marker exists at ${DONE_FILE}"
  exit 0
fi
