...
DONE_FILE="${BASE_OUTPUT_DIR}/.painn.done"
CKPT_PATH="${BASE_OUTPUT_DIR}/painn_torch.pth"
LOG_FILE="${BASE_LOG_DIR}/5_painn.log"

if [[ -f "${DONE_FILE}" && -f "${CKPT_PATH}" ]]; then
  echo "Skip painn: completed marker exists at ${DONE_FILE}"
  exit 0
fi
