...
DONE_FILE="${BASE_OUTPUT_DIR}/.mace.done"
CKPT_PATH="${BASE_OUTPUT_DIR}/mace_torch.pth"
LOG_FILE="${BASE_LOG_DIR}/7_mace.log"

if [[ -f "${DONE_FILE}" && -f "${CKPT_PATH}" ]]; then
  echo "Skip mace: completed marker exists at ${DONE_FILE}"
  exit 0
fi
