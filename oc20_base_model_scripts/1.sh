...
python "${PROJECT_ROOT}/train_oc20_base_multigpu.py" \
  ...
  --output-dir "${OUTPUT_ROOT}/dp" \
  --save-name "dp_compact.pth" \
  --model-kwarg "hidden_channels=96" \
  --model-kwarg "descriptor_embed_dim=48" \
  --model-kwarg "descriptor_axis_dim=12" \
  --model-kwarg "descriptor_type_emb_dim=12" \

  2>&1 | tee "${LOG_ROOT}/1_dp_compact.log"
