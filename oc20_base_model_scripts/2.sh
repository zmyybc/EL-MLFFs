...
python "${PROJECT_ROOT}/train_oc20_base_multigpu.py" \
  ...
  --output-dir "${OUTPUT_ROOT}/nep" \
  --save-name "nep_compact.pth" \
  --model-kwarg "hidden_channels=96" \
  --model-kwarg "n_max_radial=8" \
  --model-kwarg "n_max_angular=4" \
  --model-kwarg "basis_radial=8" \
  --model-kwarg "basis_angular=6" \

  2>&1 | tee "${LOG_ROOT}/2_nep_compact.log"
