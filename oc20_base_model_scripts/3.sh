...
python "${PROJECT_ROOT}/train_oc20_base_multigpu.py" \
  ...
  --output-dir "${OUTPUT_ROOT}/mtp" \
  --save-name "mtp_compact.pth" \
  --model-kwarg "hidden_channels=96" \
  --model-kwarg "descriptor_num_basis=12" \
  --model-kwarg "descriptor_type_emb_dim=8" \
  --model-kwarg "descriptor_radial_hidden_dim=48" \

  2>&1 | tee "${LOG_ROOT}/3_mtp_compact.log"
