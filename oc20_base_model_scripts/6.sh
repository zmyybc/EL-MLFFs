...
python "${PROJECT_ROOT}/train_oc20_base_multigpu.py" \
  ...
  --output-dir "${OUTPUT_ROOT}/schnet" \
  --save-name "schnet_compact.pth" \
  --model-kwarg "hidden_channels=96" \
  --model-kwarg "num_filters=96" \
  --model-kwarg "num_interactions=3" \
  --model-kwarg "num_gaussians=40" \

  2>&1 | tee "${LOG_ROOT}/6_schnet_compact.log"
