...
python "${PROJECT_ROOT}/train_oc20_base_multigpu.py" \
  ...
  --output-dir "${OUTPUT_ROOT}/mace" \
  --save-name "mace_compact.pth" \
  --model-kwarg "num_layers=2" \
  --model-kwarg "num_basis=16" \
  --model-kwarg "hidden_scalar_dim=64" \
  --model-kwarg "hidden_vector_dim=32" \

  2>&1 | tee "${LOG_ROOT}/7_mace_compact.log"
