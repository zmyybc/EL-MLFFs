...
if [[ ! -f "${WORK_DIR}/${TRAIN_FILE}" ]]; then
  echo "Missing train file: ${WORK_DIR}/${TRAIN_FILE}" >&2
  exit 1
fi
if [[ ! -f "${WORK_DIR}/${VAL_FILE}" ]]; then
  echo "Missing val file: ${WORK_DIR}/${VAL_FILE}" >&2
  exit 1
fi
...
echo "Launch old-data connected conservative meta"
echo "models=${BASE_MODELS[*]}"
echo "nproc=${NPROC_PER_NODE} gpus=${CUDA_VISIBLE_DEVICES} master_port=${MASTER_PORT}"
echo "train=${WORK_DIR}/${TRAIN_FILE}"
echo "val=${WORK_DIR}/${VAL_FILE}"
