...
LOG_FILE="${LOG_DIR}/09_meta_direct.log"
SAVE_PATH="${META_OUTPUT_DIR}/direct_meta_7model.pth"
AUTO_BATCH_ARGS=()
...
echo "Launch peptide DFT direct meta: nproc=${NPROC_PER_NODE} gpus=${CUDA_VISIBLE_DEVICES} data=${DATA_FILE}"
python -m torch.distributed.run \
  --standalone \
  --nproc_per_node="${NPROC_PER_NODE}" \
  "${PROJECT_ROOT}/train_direct_meta_with_bases.py" \
